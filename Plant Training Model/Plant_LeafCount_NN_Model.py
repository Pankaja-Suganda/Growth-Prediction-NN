...
Y = np.array(Data[['Leaf_Count']].values)

GdataX = np.array(Data_['Day'].values).reshape(5,8,36)
GdataY = np.array(Data_['Leaf_Count'].values).reshape(5,8,36)



# Graphs Plotting Function (Existing Data)
def Graph_Subplot(dataX,dataY,Xpred,group):
    ...
    plt.xlabel('Days')
    plt.ylabel('Leaf_Count')
    plt.legend(loc='upper left')  
    plt.show()

    return YPred

# Graph Plotting Function (Predicted Data)
...
train = tf.compat.v1.train.ProximalAdagradOptimizer(0.3).minimize(e)
